...
const ListingDetailsBelowSectionTwo = (data) => {
  ...
  return (
    <div className="flex flex-col border-[1px] border-gray-100 rounded-lg p-4 shadow-md ">
        ...
        <h1 className="text-2xl lg:text-4xl font-bold text-blue-600">  {new Intl.NumberFormat('en-PK', { style: 'currency', currency: 'PKR' }).format(listingDetails?.price)}</h1> 
        <h1 className="text-gray-600 text-xs font-semibold">{formattedDate}</h1>
        </div>
        <h1  className="text-lg lg:text-2xl font-semibold mb-2 ">{listingDetails?.title}</h1>
        <p className="flex gap-2"><MapPin className="text-blue-600"/>{listingDetails?.location?.city.charAt(0).toUpperCase()
